...
import { getPropertyBySlug, getRelatedProperties } from "@/drizzle/queries/properties";
import { getCdnImageUrl } from "@/lib/utils";
import { SITE_NAME } from "@/lib/seo";
import { Bed, Bath, Maximize, MapPin, ArrowLeft, DownloadIcon, ArrowRight } from "lucide-react";
    ...
    <main>
      ...
      <section className="border-t border-border/60 bg-background py-10 lg:py-14">
        <div className="container mx-auto px-4 lg:px-8">
          <Button variant="outline" className="border-foreground/25 bg-transparent" asChild>
            <Link href="/home-2/properties">
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-3 lg:gap-16">
            <div className="lg:col-span-2">
              {imageUrls.length > 0 ? (
                <PropertyImageGallery images={imageUrls} title={property.title} />
              ) : null}
              ...
              <div className="mb-10 grid gap-3 border-y border-border/60 py-5 sm:grid-cols-3">
                <div className="inline-flex items-center gap-2 text-sm text-muted-foreground">
                  <Bed className="h-4 w-4" aria-hidden /> {property.bedrooms} bedrooms
                </div>
                <div className="inline-flex items-center gap-2 text-sm text-muted-foreground">
                  <Bath className="h-4 w-4" aria-hidden /> {property.bathrooms} bathrooms
                </div>
                <div className="inline-flex items-center gap-2 text-sm text-muted-foreground">
                  <Maximize className="h-4 w-4" aria-hidden /> {formatArea(property.area)}
                </div>
              </div>
